...
parse_one (T_LParen : rest) = parse_list_body rest
    where
        ...
        parse_list_body toks = let (obj, rest) = parse_one toks in (C_List $ C_Cons obj (parse_list_body toks), rest)
-- substitute quote syntax for (quote something)
parse_one (T_Quote : rest) = let (obj, rest) = parse_one rest in (C_List $ C_Cons (C_Symbol "quote") $ C_Cons obj $ C_EmptyList, rest)
